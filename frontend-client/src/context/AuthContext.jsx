...
export const AuthProvider = ({ children, requiredRole = null }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      clearAuth();
      let message = err.response?.data?.message || err.message || 'Erreur de connexion';
      
      // Gérer spécifiquement les erreurs de rate limiting
      if (err.response?.status === 429) {
        message = message || 'Trop de tentatives de connexion. Veuillez patienter quelques instants avant de réessayer.';
      }
  ...
  const changePassword = async (currentPassword, newPassword) => {
    try {
      ...
      const message = error.response?.data?.message || error.message || 'Erreur de modification';
      toast.error(message);
      return { success: false, error: message };
    }
  };
